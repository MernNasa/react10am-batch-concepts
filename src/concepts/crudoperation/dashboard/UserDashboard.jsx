    ...
    <div className='w-full h-[89vh]  flex'>
      <div className='w-[20%] h-full bg-white'>
                ...
                dashboardlist.map((ele,index)=>{
                  return(
                    <li key={index+1} className=' w-[80%] text-center rounded-lg font-bold text-lg tracking-widest py-3 '>
                      <NavLink className="flex w-full h-full items-center justify-start gap-3 ps-3 text-gray-600 capitalize" to={ele.path}>{ele.icon} {ele.text}</NavLink>
                    </li>
                  )
                })
              }
          ...
          <div className='w-full h-25  flex items-center justify-center'>
            <button className='px-15 py-3 rounded-lg text-lg font-light  border border-black flex items-center gap-3 bg-[#7E3AF2] text-white cursor-pointer' onClick={logout}><IoMdLogOut / > Logout</button>
          </div>
      </div>
